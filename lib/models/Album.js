...
class Album {
  ...
  static async updateAlbumById(id, attrs) {
    const album = await Album.getAlbumById(id);
    if (!album) return null;
    const { artist_name, album_title, album_genres } = {
      ...album,
      ...attrs,
    };
    const { rows } = await pool.query(
      'UPDATE albums SET artist_name=$2, album_title=$3, album_genres=$4 WHERE id=$1 RETURNING *',
      [id, artist_name, album_title, album_genres]
    );
